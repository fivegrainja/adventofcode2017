...
long_bridges = []
for b in all_bridges:
    if len(b) > long_length:
        long_length = len(b)
        long_bridges = [b]
    elif len(b) == long_length:
        long_bridges.append(b)

print('longest length: %d' % long_length)
print('num at that length: %d' % len(long_bridges))

strong_bridge = None
strong_strength = -1
for b in long_bridges:
    s = strength(b)
    if s > strong_strength:
        strong_bridge = b
        strong_strength = s

#print('num bridges: %d' % len(long_bridges))
